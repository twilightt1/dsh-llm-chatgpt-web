...
function compilePrompt(options, maxChars, notice, native) {
	...
	if (native !== void 0) {
		contract.push(`Use the attached ${JSON.stringify(native.connectorName)} connector.`, `First call dsh_round_start with request_id ${native.requestId}.`, "Then use dsh_tool_inventory and dsh_tool_call with that same request_id.", "If the task asks about a local repository, files, commands, environment, or any other tool-backed fact, you MUST use the connector before answering.", "Only connector-backed tool results are evidence that an action ran. Never claim a command or tool ran from memory or inference.", "Never reveal request_id in the answer.");
...
async function deleteOwnedConversation(page, conversationId, timeoutMs = DEFAULT_DELETE_TIMEOUT_MS) {
	...
	if (!await visible(optionsButton)) throw new LlmError("ChatGPT owned conversation has no conversation-options control.", "PROVIDER_ERROR");
	await optionsButton.click({ force: true });
	const deleteButton = page.getByTestId("delete-chat-menu-item").last();
	if (!await visible(deleteButton)) throw new LlmError("ChatGPT owned conversation has no delete action.", "PROVIDER_ERROR");
	await deleteButton.click({ force: true });
	const confirmButton = page.getByTestId("delete-conversation-confirm-button").last();
	if (!await visible(confirmButton)) throw new LlmError("ChatGPT owned conversation deletion has no confirmation action.", "PROVIDER_ERROR");
...
async function retryPendingConversationDeletions(page, ledger, timeoutMs = DEFAULT_DELETE_TIMEOUT_MS) {
	for (const conversationId of ledger.pending()) {
		...
		if (conversationIdFromUrl(page.url()) === conversationId) await deleteOwnedConversation(page, conversationId, timeoutMs);
...
async function* streamTextTurn(page, options) {
	...
	const notifyConversationCreated = () => {
		...
		if (conversationId === void 0) throw new LlmError("ChatGPT connector turn did not expose a stable conversation ID after submission.", "PROVIDER_ERROR");
