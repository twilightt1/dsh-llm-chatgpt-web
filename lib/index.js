...
import { i as NativeSafetyError, r as NativePolicyDeniedError, t as NativeBrokerSocketServer } from "./chunks/broker-socket-5tKiwTN6.js";
...
function createWorkspaceBoundary(config, privatePaths = []) {
	...
	const sensitiveDigest = hashCanonical("native-sensitive-rules", 1, {
		builtInVersion: 1,
	...
	try {
		for (const message of projected) {
			for (const block of message.content) if (block.type === "tool-call") {
				...
				block.arguments = canonicalJson(decision.binding.pathArguments.length === 0 ? decision.arguments : boundary.rebaseProviderArguments(decision.arguments, decision.binding.pathArguments));
