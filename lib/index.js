...
function evidenceForView(view) {
	...
	if (resultMessages.length < view.calls.length) return {
		kind: "missing",
	...
	for (const [index, call] of view.calls.entries()) {
		...
		if (view.matchesResult !== void 0 && !view.matchesResult(result, index)) return {
			kind: "conflicting",
			reason: "result-evidence-conflicting"
		};
		results.push(result);
	}
	if (resultMessages.slice(view.calls.length).some((message) => message.source.kind === "tool" || message.content.some((block) => block.type === "tool-result"))) return {
		kind: "conflicting",
...
function assessNativeResultEvidence(input) {
	...
	if (provider.kind === "missing") return {
		kind: "missing",
		reason: "provider-view-missing"
	};
	if (provider.kind === "conflicting") return {
		kind: "conflicting",
		reason: "provider-view-conflicting"
	};
	if (!sameEvidenceCalls(input.canonical.calls, input.provider.calls) || provider.results.length !== canonical.results.length || provider.results.some((result, index) => result.isError !== canonical.results[index]?.isError)) return {
		kind: "conflicting",
		reason: "result-evidence-conflicting"
	};
	return {
