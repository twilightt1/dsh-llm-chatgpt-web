...
import { t as NativeBrokerSocketServer } from "./chunks/broker-socket-BrvGePxq.js";
...
function compilePrompt(options, maxChars, notice, native) {
	...
	if (native !== void 0) {
		contract.push(`Use the attached ${JSON.stringify(native.connectorName)} connector.`, `First call dsh_round_start with request_id ${native.requestId}.`, "Then use dsh_tool_inventory and dsh_tool_call with that same request_id.", "Never reveal request_id in the answer.");
...
async function waitForExactConnectorRow(page, connectorName, deadline) {
	let lastError;
	for (;;) {
...
async function assertConnectorPill(page, connectorName, deadline) {
	for (;;) {
...
async function selectChatGptConnector(page, connectorName, signal) {
	...
	for (let attempt = 0; attempt < 3; attempt += 1) {
		...
		try {
			...
			const row = await waitForExactConnectorRow(page, connectorName, Date.now() + 1e4);
			throwIfAborted$1(signal);
			await row.press("Enter");
			await visibleComposer(page);
			await assertConnectorPill(page, connectorName, Date.now() + 1e4);
			return;
		} catch (error) {
			lastError = error;
	...
	*/
	...
	async function* captureRound(responseIdentity) {
		...
		for (;;) {
			...
			if (healthError !== void 0) throw new LlmError(healthError, "PROVIDER_ERROR");
...
var ChatGptWebAdapter = class extends LlmAdapter {
	...
	*/
	...
	async *runTurn(options) {
		...
		if (nativeRequested && options.sessionId === void 0) throw new LlmError("Native MCP transport requires a sessionId for round ownership.", "INVALID_REQUEST");
		if (nativeRequested && this.config.native === void 0) throw new LlmError("Native MCP transport is not initialized by the plugin runtime.", "UNSUPPORTED");
		...
		const cleanup = (mode) => {
			...
			cleanupPromise = (async () => {
				if (mode === "stop" && page !== void 0 && !page.isClosed()) await page.locator("[data-testid=\"stop-button\"]").last().press("Enter").catch(() => {});
				try {
		...
		try {
			...
			const prompt = compilePrompt(options, COMPOSER_CHAR_BUDGET, this.takeNotice(options), lease === void 0 ? void 0 : {
			...
			if (turnResult.kind === "tool-batch") {
				...
				if (lease === void 0) throw new LlmError("Native tool batch returned without a native lease.", "TRANSPORT");
...
var NativeToolBroker = class {
	...
	register(input) {
		...
		if (!Number.isFinite(input.ttlMs) || input.ttlMs <= 0) throw new Error("native broker TTL must be a positive finite number");
		if (!Number.isSafeInteger(input.invocationTimeoutMs) || input.invocationTimeoutMs <= 0) throw new Error("native broker invocation timeout must be a positive safe integer");
...
var NativeLease = class {
	...
	record;
	requestId;
	constructor(owner, record, requestId) {
		this.owner = owner;
		this.record = record;
	...
	async fail(cleanup, cause) {
		this.assertOpen();
		if (this.record.state === "parked" && this.record.cleanup !== cleanup) throw new Error("native parked step lease already owns a cleanup callback");
	...
	assertOpen() {
		...
		if (this.owner.currentRecord() !== this.record) throw new Error("native step lease is no longer owned by the coordinator");
	}
};
...
function resolveAdapterOptions(config) {
	...
	if (connectorTransport === "mcp" && process.platform === "win32") throw new Error("llm-chatgpt-web: connectorTransport \"mcp\" is unsupported on win32; use text transport");
	const mcpInvocationTimeoutMs = config.mcpInvocationTimeoutMs ?? 9e4;
	if (!Number.isSafeInteger(mcpInvocationTimeoutMs) || mcpInvocationTimeoutMs < 1) throw new Error("llm-chatgpt-web: mcpInvocationTimeoutMs must be a positive safe integer");
	return {
		profileDir,
