...
function hasExactNativeToolResults(claim, options) {
	const baseLength = (claim.canonicalRequest ?? claim.request).messages.length;
	const incomingAssistant = options.messages[baseLength];
	if (incomingAssistant === void 0 || !sameMessages([incomingAssistant], [claim.canonicalAssistantMessage ?? claim.assistantMessage]) || !assistantCallsMatch(incomingAssistant, claim.pendingCalls)) return false;
