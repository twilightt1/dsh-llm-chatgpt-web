...
function compilePrompt(options, maxChars, notice, native) {
	...
	if (native !== void 0 && options.messages.some((message) => message.source.kind === "tool")) sections.push("[Native continuation] The DSH tool call(s) in the JSON context have already been executed. Their tool_result content is authoritative: answer the original user from those results and do not call the connector again for those same calls. Only make another connector call if the original request explicitly needs a fact that is not in the results.");
