...
import { S as durableAtomicWritePrivateFile, T as syncPrivateDirectory, _ as loadManagedNativeRuntimeConfig, a as shellQuotePosix, b as assertPrivateRegularFile, c as ManagedRuntimeConfigurationError, h as defaultNativeRuntimeConfigPath, i as requireNativeApproval, l as ManagedRuntimeTransportError, n as formatNativeApprovalChallenge, o as canonicalJson, r as readNativeApprovalChallenge, s as hashCanonical, t as approveNativeChallenge, u as ManagedTunnelRuntime, x as atomicWritePrivateFile } from "./chunks/grants-VS-D2N-F.js";
...
import { closeSync, existsSync, fstatSync, lstatSync, mkdirSync, openSync, readFileSync, readSync, realpathSync, renameSync, statSync, writeFileSync } from "node:fs";
...
function hasExactNativeToolResults(claim, options) {
	...
	if (incomingAssistant === void 0 || !sameMessages(options.messages.slice(0, baseLength), canonicalRequest.messages) || !sameMessages([incomingAssistant], [claim.canonicalAssistantMessage ?? claim.assistantMessage]) || !assistantCallsMatch(incomingAssistant, claim.pendingCalls)) return false;
...
function decideNativeContinuation(claim, options, identity) {
	...
	if (incomingAssistant === void 0 || !sameMessages([incomingAssistant], [claim.assistantMessage])) {
		if (freshReason !== void 0) return claim.physicalAvailable ? {
			kind: "fresh-replay",
			reason: freshReason
	...
	async *runPersistentNativeTurn(options, connection, prepared) {
		...
		try {
			...
			const cleanup = (mode) => {
				...
				if (page === void 0 && session === void 0) return Promise.resolve();
			...
			else if (activeCleanup !== void 0 && active === void 0) await activeCleanup(options.signal?.aborted ? "stop" : "close").catch(() => {});
			throw failure;
		}
	}
...
var RoundRecord = class {
	...
	retired = false;
	resumeWaiter;
	constructor(sessionId, requestId, snapshot, hooks) {
...
var NativeRoundCoordinator = class {
	...
	createRecord(sessionId, requestId, snapshot) {
		let record;
		record = new RoundRecord(sessionId, requestId, snapshot, {
			broker: this.broker,
...
//#endregion
export { ChatGptWebAdapter, Config, NativeApprovalRequiredError, NativePolicyDeniedError, NativeRoundCoordinator, NativeSafetyError, NativeToolBroker, PROVIDER, apply, approveNativeChallenge, compilePrompt, correlateToolResults, defaultBrokerSocketPath, durableAtomicWritePrivateFile, formatNativeApprovalChallenge, inject, name, readNativeApprovalChallenge, requireNativeApproval, resolveAdapterOptions, shellQuotePosix, syncPrivateDirectory };
