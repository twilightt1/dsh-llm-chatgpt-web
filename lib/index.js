...
function parseToolCallsWithSchemas(text, knownToolSchemas) {
	...
	for (;;) {
		...
		if (!isRecord$1(parsed) || typeof parsed["name"] !== "string") {
			rejected.push({
				raw: body.slice(0, 200),
		...
		if (!isRecord$1(parsed["arguments"])) {
			rejected.push({
				raw: body.slice(0, 200),
...
function canonicalJson(value) {
	const normalize = (input, inArray = false) => {
		if (input === void 0) return inArray ? null : void 0;
		...
		for (const key of Object.keys(input).sort()) {
			const child = normalize(input[key]);
...
*/
function nativeExecutionKey(options) {
...
function hasExactNativeToolResults(claim, options) {
	...
	if (incomingAssistant === void 0 || !sameMessages(options.messages.slice(0, baseLength), claim.request.messages) || !sameMessages([incomingAssistant], [claim.assistantMessage]) || !assistantCallsMatch(incomingAssistant, claim.pendingCalls)) return false;
...
*/
function decideNativeContinuation(claim, options) {
	...
	if ((claim.requestKey ?? nativeExecutionKey(claim.request)) !== nativeExecutionKey(claim.request)) return failure("INVALID_REPLAY_STATE", "native parked response has an invalid logical request identity");
	...
	if (!assistantCallsMatch(incomingAssistant, claim.pendingCalls)) return failure("CALL_MISMATCH", "native continuation assistant tool calls do not match the parked broker calls");
...
function nativeClaim(options, response, chunks) {
...
var ChatGptWebAdapter = class extends LlmAdapter {
	...
	*/
	...
	async *runPersistentNativeTurn(options, connection) {
	...
	async *runTurn(options) {
		...
		if (nativeRound && this.config.native === void 0) throw new LlmError("Native MCP transport is not initialized by the plugin runtime.", "UNSUPPORTED");
		if (nativeTools) {
