...
function freezeEvidence(value) {
	return deepFreeze$3(value);
}
function evidenceForView(view) {
	let boundaryIndex;
	for (const [index, message] of view.messages.entries()) {
		if (!view.matchesAssistant(message)) continue;
		if (boundaryIndex !== void 0) return freezeEvidence({
			kind: "ambiguous",
			reason: "assistant-boundary-ambiguous"
		});
		boundaryIndex = index;
	}
	if (boundaryIndex === void 0) return freezeEvidence({
		kind: "missing",
		reason: "assistant-boundary-missing"
	});
	const resultMessages = view.messages.slice(boundaryIndex + 1);
	if (resultMessages.length < view.calls.length) return freezeEvidence({
		kind: "missing",
	...
	for (const [index, call] of view.calls.entries()) {
		...
		if (result === void 0) return freezeEvidence({
			kind: "missing",
			reason: "result-malformed"
		});
		if (view.matchesResult !== void 0 && !view.matchesResult(result, index)) return freezeEvidence({
			kind: "conflicting",
			reason: "result-evidence-conflicting"
		});
		results.push(result);
	}
	if (resultMessages.slice(view.calls.length).some((message) => message.source.kind === "tool" || message.content.some((block) => block.type === "tool-result"))) return freezeEvidence({
		kind: "conflicting",
...
function sameEvidenceCalls(left, right) {
	if (left.length !== right.length) return false;
	return left.every((call, index) => {
		const other = right[index];
		return other !== void 0 && String(call.callId) === String(other.callId) && call.name === other.name;
	});
}
/** Prove exact durable results across canonical and optional provider views. */
function assessNativeResultEvidence(input) {
	const canonical = evidenceForView(input.canonical);
	if (canonical.kind !== "proven") return canonical;
	if (input.provider === void 0) return canonical;
	const provider = evidenceForView(input.provider);
	if (provider.kind === "ambiguous") return provider;
	if (provider.kind === "missing") return freezeEvidence({
		kind: "missing",
		reason: "provider-view-missing"
	});
	if (provider.kind === "conflicting") return freezeEvidence({
		kind: "conflicting",
		reason: "provider-view-conflicting"
	});
	if (!sameEvidenceCalls(input.canonical.calls, input.provider.calls) || provider.results.length !== canonical.results.length || provider.results.some((result, index) => result.isError !== canonical.results[index]?.isError)) return freezeEvidence({
		kind: "conflicting",
		reason: "result-evidence-conflicting"
	});
	return freezeEvidence({
...
function correlateNativeToolResults(messages, calls) {
...
function assessNativeClaimResultEvidence(claim, canonicalOptions, providerOptions) {
	...
	const canonical = {
		...
		matchesAssistant: (message) => sameMessages([message], [canonicalAssistant]) && assistantCallsMatch(message, claim.pendingCalls)
	};
	const provider = providerOptions === void 0 ? void 0 : {
		messages: providerOptions.messages,
		calls: claim.providerPendingCalls ?? claim.pendingCalls,
		matchesAssistant: (message) => sameMessages([message], [claim.assistantMessage]) && assistantCallsMatch(message, claim.providerPendingCalls ?? claim.pendingCalls)
	};
	return assessNativeResultEvidence({
		canonical,
	...
	async *runPersistentNativeTurn(options, connection, prepared) {
		...
		try {
			if (active !== void 0) {
				...
				const durableResults = (uncertainOutcome ? void 0 : assessNativeClaimResultEvidence(active.claim, options, providerOptions))?.kind === "proven";
...
const correlateToolResults = correlateNativeToolResults;
