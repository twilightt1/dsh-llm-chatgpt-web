#!/usr/bin/env node
import { a as loadManagedNativeRuntimeConfig, c as assertPrivateRegularFile, d as snapshotPrivateFile, i as ensureManagedRuntimeDirectories, l as atomicWritePrivateFile, n as defaultManagedRuntimePaths, o as parseManagedNativeRuntimeConfig, s as assertPrivateDirectory, t as MANAGED_TUNNEL_CLIENT_VERSION, u as ensurePrivateDirectory } from "./chunks/runtime-config-nQMeyFzs.js";
...
function validateExistingInstallation(binaryPath, manifestPath, run) {
...
function redactTunnelDetail(value) {
	return textValue(value).replace(/tunnel_[a-f0-9]{32}/gi, "[tunnel-id]").replace(/sk-[A-Za-z0-9_-]{12,}/g, "[redacted-key]").replace(/Bearer\s+[A-Za-z0-9._~-]{12,}/gi, "Bearer [redacted-token]").slice(0, MAX_DETAIL_CHARS);
}
...
function parseNativeSetupArgs(args) {
	...
	if (command !== "setup" && command !== "doctor" && command !== "stop") throw new Error("command must be setup, doctor, or stop");
...
async function stopManagedNativeRuntime(options) {
	...
	await new ManagedTunnelRuntime({
		config: loadManagedNativeRuntimeConfig(configPath, { connectorName: options.connectorName }),
		nodeExecutable: process.execPath,
		mcpEntrypoint: resolveMcpEntrypoint(void 0),
