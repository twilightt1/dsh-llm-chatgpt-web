...
function boundedErrorMessage(error) {
	...
	return normalized.length <= 2e3 ? normalized : `${normalized.slice(0, 1997)}...`;
}
function rpcError(error) {
	if (error instanceof NativePolicyDeniedError) return {
		code: "NATIVE_POLICY_DENIED",
...
var NativeBrokerSocketServer = class {
	...
	async startListening() {
		...
		if (typeof process.getuid === "function" && parent.uid !== process.getuid()) throw new Error("unsafe broker directory ownership: expected the current user");
		let existing;
		try {
			...
			if (newline < 0) {
				if (buffered.length > this.maxLineBytes) {
					...
					this.writeResponse(socket, {
						id: "unknown",
						error: rpcError(/* @__PURE__ */ new Error("native broker request line is too large"))
					});
				}
				return;
			}
	...
	writeResponse(socket, response) {
		...
		if (Buffer.byteLength(line) > this.maxLineBytes) {
			socket.end(`${JSON.stringify({
				id: response.id,
				error: rpcError(/* @__PURE__ */ new Error("native broker response is too large"))
			})}\n`);
			return;
		}
		socket.end(line);
			...
			socket.on("data", (chunk) => {
				...
				try {
					...
					if (hasError) {
						...
						if (!isRecord(rpc) || rpc.code !== "NATIVE_POLICY_DENIED" && rpc.code !== "BROKER_FAILURE" || typeof rpc.message !== "string" || typeof rpc.releaseRound !== "boolean" || rpc.code === "NATIVE_POLICY_DENIED" && rpc.releaseRound !== false || rpc.code === "BROKER_FAILURE" && rpc.releaseRound !== true) throw new Error("native broker RPC error payload is invalid");
					}
					response = parsed;
				} catch (error) {
				...
				if (response.error !== void 0) {
					if (response.error.code === "NATIVE_POLICY_DENIED") finishReject(new NativePolicyDeniedError(response.error.message));
					else {
						const failure = new Error(response.error.message);
						Object.assign(failure, {
							code: response.error.code,
							releaseRound: response.error.releaseRound
						});
...
//#endregion
export { NativeSafetyError as i, createBrokerRpcClient as n, NativePolicyDeniedError as r, NativeBrokerSocketServer as t };
